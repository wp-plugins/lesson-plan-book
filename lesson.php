...
define('WP_CALENDAR_CUSTOM_TABLE', $table_prefix . 'calendar_custom');
...
function calendar_menu() 
{
  ...
  // Add the admin panel pages for Calendar. Use permissions pulled from above
   if (function_exists('add_menu_page')) 
     {
       add_menu_page(__('Lessons','calendar'), __('Lessons','calendar'), $allowed_group, 'calendar', 'edit_calendar');
     }
   if (function_exists('add_submenu_page')) 
     {
       add_submenu_page('calendar', __('Manage Lessons','calendar'), __('Manage Lessons','calendar'), $allowed_group, 'calendar', 'edit_calendar');
       add_action( "admin_head", 'calendar_add_javascript' );
       // Note only admin can change calendar options
       add_submenu_page('calendar', __('Manage Classes','calendar'), __('Manage Classes','calendar'), 'manage_options', 'calendar-categories', 'manage_categories');
       add_submenu_page('calendar', __('Lesson Config','calendar'), __('Lesson Options','calendar'), 'manage_options', 'calendar-config', 'edit_calendar_config');
     }
}
  ...
  if ( $new_install == true )
    {
      $sql = "CREATE TABLE " . WP_CALENDAR_TABLE . " (
              ...
              event_title varchar(230) NOT NULL,
              event_assessment text NOT NULL,
              ...
              event_resources1 varchar(254) default NULL,
              event_resources2 varchar(254) default NULL,
              event_resources3 varchar(254) default NULL,
              event_objective text,
              event_link1 varchar(254) default NULL,
              event_link2 varchar(254) default NULL,
              event_link3 varchar(254) default NULL,
      ...
      $sql = "CREATE TABLE " . WP_CALENDAR_CUSTOM_TABLE . " (
      ...
      $wpdb->get_results($sql);
      $sql = "INSERT INTO ".WP_CALENDAR_CONFIG_TABLE." SET config_item='display_jump', config_value='true'";
      $wpdb->get_results($sql);
      $sql = "INSERT INTO ".WP_CALENDAR_CONFIG_TABLE." SET config_item='display_todays', config_value='false'";
      $wpdb->get_results($sql);
      $sql = "INSERT INTO ".WP_CALENDAR_CONFIG_TABLE." SET config_item='display_upcoming', config_value='false'";
      $wpdb->get_results($sql);
      ...
      $wpdb->get_results($sql);
      $sql = "INSERT INTO ".WP_CALENDAR_CONFIG_TABLE." SET config_item='enable_categories', config_value='true'";
      $wpdb->get_results($sql);
...
function wp_events_display_list()
{
	...
	if ( !empty($events) )
	{
		...
		<table class="widefat page fixed" width="100%" cellpadding="3" cellspacing="3">
		        ...
		        <th class="manage-column" scope="col"><?php _e('Class','calendar') ?></th>
...
function wp_events_edit_form($mode='add', $event_id=false)
{
	...
	// Deal with possibility that form was submitted but not saved due to error - recover user's entries here
     ...
     $WhareClause = "";
     if ( $event_id !== false ){
         $sql = "SELECT * FROM " . WP_CALENDAR_CUSTOM_TABLE . " WHERE event_id='".$event_id."'";
         $customfields = $wpdb->get_results($sql);
         foreach($customfields as $customfield)
         {
             
			 $divIdName = "Where".$theValue."Div";
			 $custom_key =  $customfield->custom_key;
             $custom_value =  $customfield->custom_value;
             $WhareClause.= "<div id=\"$divIdName\"><table cellpadding=\"4\" cellspacing=\"0\" width=\"100%\" border=\"0\"><tr><td valign=\"top\">Key:</td><td valign=\"top\"><input type=\"text\" name=\"custom_key".$theValue."\" id=\"custom_key".$theValue."\" value=\"".$custom_key."\" size=30></td><td valign=\"top\">&nbsp;&nbsp;&nbsp;Value:</td><td valign=\"top\"><textarea rows=\"2\" cols=\"30\" NAME=\"custom_val".$theValue."\" id=\"custom_val".$theValue."\">$custom_value</textarea></td><td valign=\"top\"><input type=\"button\" name=\"Add\" value=\"+\" onClick=\"addMore();\"><a href=\"javascript:;\" onclick=\"removeEvent(\'".$divIdName."\')\"><input type=\"button\" name=\"btn\" value=\"-\"></a></td></tr></table></div>";
...
function addMore() {
      var WhereClause = "";
      var ni     = document.getElementById('WhereDiv');
      var numi   = document.getElementById('theValue');
      var num    = (document.getElementById("theValue").value -1)+ 2;
      numi.value = num;
      var divIdName = "Where"+num+"Div";
      var newdiv = document.createElement('div');
      newdiv.setAttribute("id",divIdName);
      var WhareClause = "";
      WhareClause =  "<table cellpadding=\"4\" cellspacing=\"0\" width=\"100%\" border=\"0\"><tr><td valign=\"top\">Key:</td><td valign=\"top\"><input type=\"text\" name=\"custom_key"+num+"\" id=\"custom_key"+num+"\" size=30></td><td valign=\"top\">&nbsp;&nbsp;&nbsp;Value:</td><td valign=\"top\"><textarea rows=\"2\" cols=\"30\" NAME=\"custom_val"+num+"\" id=\"custom_val"+num+"\"></textarea></td><td valign=\"top\"><input type=\"button\" name=\"Add\" value=\"+\" onClick=\"addMore();\"><a href=\"javascript:;\" onclick=\"removeEvent(\'"+divIdName+"\')\"><input type=\"button\" name=\"btn\" value=\"-\"></a></td></tr></table>"
      newdiv.innerHTML = WhareClause;
      ni.appendChild(newdiv);
}

function removeEvent(divNum) {
      var d = document.getElementById('WhereDiv');
...
tinyMCE.init({
	...
	<form name="quoteform" id="quoteform" class="wrap" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=calendar" enctype="multipart/form-data">
		...
		<div id="linkadvanceddiv" class="postbox">
			<div style="float: left; width: 98%; clear: both;" class="inside">
                                ...
                                <tr>				
				<td><legend><?php _e('Title','calendar'); ?></legend></td>
				<td><input type="text" name="event_title" class="input" size="40" maxlength="230"
					value="<?php if ( !empty($data) ) echo htmlspecialchars($data->event_title); ?>" /></td>
                                </tr>
                                
								<tr>
				<td style="vertical-align:top;"><legend><?php _e('Objective','calendar'); ?></legend></td>
				<td><textarea name="event_objective" class="input" rows="10" cols="90"><?php if ( !empty($data) ) echo htmlspecialchars($data->event_objective); ?></textarea></td>
                                </tr>

                                <tr>
				<td style="vertical-align:top;"><legend><?php _e('Description','calendar'); ?></legend></td>
				<td><textarea name="event_desc" class="input" rows="10" cols="90"><?php if ( !empty($data) ) echo htmlspecialchars($data->event_desc); ?></textarea></td>
                                </tr>
								<tr>
				<td style="vertical-align:top;"><legend><?php _e('Assessment/Evaluation','calendar'); ?></legend></td>
				<td><textarea name="event_assessment" class="tinymce" rows="10" cols="90"><?php if ( !empty($data) ) echo htmlspecialchars($data->event_assessment); ?></textarea></td>
                                </tr>

                                <tr>
				<td><legend><?php _e('Related Links 1','calendar'); ?></legend></td>
                                <td><input type="text" name="event_link1" class="input" size="40" value="<?php if ( !empty($data) ) echo htmlspecialchars($data->event_link1); ?>" />&nbsp;&nbsp;Must include http:// in URL</td>
                                </tr>

                                <tr>
				<td><legend><?php _e('Related Links 2','calendar'); ?></legend></td>
                                <td><input type="text" name="event_link2" class="input" size="40" value="<?php if ( !empty($data) ) echo htmlspecialchars($data->event_link2); ?>" />&nbsp;&nbsp;Must include http:// in URL</td>
                                </tr>

                                <tr>
				<td><legend><?php _e('Related Links 3','calendar'); ?></legend></td>
                                <td><input type="text" name="event_link3" class="input" size="40" value="<?php if ( !empty($data) ) echo htmlspecialchars($data->event_link3); ?>" />&nbsp;&nbsp;Must include http:// in URL</td>
                                </tr>


								<tr>
				<td style="vertical-align:top;"><legend><?php _e('Resources/Files 1','calendar'); ?></legend></td>
				<td><input type="file" name="event_resources1" /><?php if ( !empty($data) ) echo htmlspecialchars($data->event_resources1); ?></td>
                                </tr>

								<tr>
				<td style="vertical-align:top;"><legend><?php _e('Resources/Files 2','calendar'); ?></legend></td>
				<td><input type="file" name="event_resources2" /><?php if ( !empty($data) ) echo htmlspecialchars($data->event_resources2); ?></td>
                                </tr>

								<tr>
				<td style="vertical-align:top;"><legend><?php _e('Resources/Files 3','calendar'); ?></legend></td>
				<td><input type="file" name="event_resources3" /><?php if ( !empty($data) ) echo htmlspecialchars($data->event_resources3); ?></td>
                                </tr>

                                <tr>
				<td><legend><?php _e('Class','calendar'); ?></legend></td>
...
<!--                                 </tr>
				...
				<td>	<input type="text" name="event_time" class="input" size=12
					...
					?>" /> <?php _e('Optional, set blank if not required.','calendar'); ?> <?php _e('Current time difference from GMT is ','calendar'); echo get_option('gmt_offset'); _e(' hour(s)'); ?>
								...
								<input type="hidden" name="event_time" value="00:00">
                                <tr>
				<td><legend><?php _e('Recurring Lesson','calendar'); ?></legend></td>
				<td>	<?php
              ...
              <tr>
                 <td colspan="2">
                     <table border="0" cellpadding="2" cellspacing="0">
                       <tr>
                          <td><a href="javascript:void(0);" onClick="addMore();">Add Custom Field</a></td>
                       </tr>
                       <tr height="10px;">
                          <td>&nbsp;</td>
                       </tr>
                       <tr>
                           <td>
                             ...
                             <input type="hidden" value="<?php echo $theValue;?>" id="theValue" />
                           </td>
                        </tr>
                     </table>
                 </td>
...
<!--
	var ni = document.getElementById('WhereDiv');
    ni.innerHTML = '<?php echo $WhareClause;?>';
//-->
</SCRIPT>
...
if ( $action == 'add' )
{
	...
	$objective = !empty($_REQUEST['event_objective']) ? $_REQUEST['event_objective'] : '';
	

	$assessment = !empty($_REQUEST['event_assessment']) ? $_REQUEST['event_assessment'] : '';
    ...
    $file1 = !empty($_FILES["event_resources1"]["name"]) ? $_FILES["event_resources1"]["name"] : '';
    if($_FILES["event_resources1"]["tmp_name"]!=''){
        list($fileName,$fileExtn)= explode(".",$file1);
        $file1 = $fileName."_".time().".".$fileExtn;
        //$file1 = $_FILES["event_resources1"]["name"];
        move_uploaded_file($_FILES["event_resources1"]["tmp_name"],"../wp-content/" . $file1);
    }

    $file2 = !empty($_FILES["event_resources2"]["name"]) ? $_FILES["event_resources2"]["name"] : '';
    if($_FILES["event_resources2"]["tmp_name"]!=''){
        list($fileName,$fileExtn)= explode(".",$file2);
        $file2 = $fileName."_".time().".".$fileExtn;
		//$file2 = $_FILES["event_resources2"]["name"];
        move_uploaded_file($_FILES["event_resources2"]["tmp_name"],"../wp-content/" . $file2);
    }

    $file3 = !empty($_FILES["event_resources3"]["name"]) ? $_FILES["event_resources3"]["name"] : '';
    if($_FILES["event_resources3"]["tmp_name"]!=''){
        list($fileName,$fileExtn)= explode(".",$file3);
        $file3 = $fileName."_".time().".".$fileExtn;
		//$file3 = $_FILES["event_resources3"]["name"];
        move_uploaded_file($_FILES["event_resources3"]["tmp_name"],"../wp-content/" . $file3);
    }
	...
	$linky1 = !empty($_REQUEST['event_link1']) ? $_REQUEST['event_link1'] : '';
	$linky2 = !empty($_REQUEST['event_link2']) ? $_REQUEST['event_link2'] : '';
	$linky3 = !empty($_REQUEST['event_link3']) ? $_REQUEST['event_link3'] : '';
	...
	if ( ini_get('magic_quotes_gpc') )
	{
		...
		$assessment = stripslashes($assessment);
		$file1 = stripslashes($file1);
		$file2 = stripslashes($file2);
		$file3 = stripslashes($file3);
		...
		$linky1 = stripslashes($linky1);	
		$linky2 = stripslashes($linky2);	
		$linky3 = stripslashes($linky3);	
	}	
	...
	if ((preg_match($date_format_one,$begin) || preg_match($date_format_two,$begin)) && (preg_match($date_format_one,$end) || preg_match($date_format_two,$end)))
	  {
	    ...
	    $end_d = $end_split[2];
		 ...
		 {
		   ?>
		   <div class="error"><p><strong><?php _e('Error','calendar'); ?>:</strong> <?php _e('Your  end date must be either after or the same as your  begin date','calendar'); ?></p></div>
		   <?php
		 }
	     } 
	    else
	...
	if (preg_match('/^[a-zA-Z0-9]{1}[a-zA-Z0-9[:space:]]{0,229}$/',$title))
	  {
	    $title_ok =1;
	  }
	else
	  {
              ?>
              <div class="error"><p><strong><?php _e('Error','calendar'); ?>:</strong> <?php _e('The title must be between 1 and 230 characters in length and contain no punctuation. Spaces are allowed but the title must not start with one.','calendar'); ?></p></div>
              <?php
	  }
	...
	if ($start_date_ok == 1 && $end_date_ok == 1 && $time_ok == 1 && $url_ok == 1 && $title_ok == 1 && $recurring_ok == 1)
	  {
	    $sql = "INSERT INTO " . WP_CALENDAR_TABLE . " SET event_title='" . mysql_escape_string($title)
	     . "', event_desc='" . mysql_escape_string($desc) . "', event_objective='" . mysql_escape_string($objective) . "', event_assessment='" . mysql_escape_string($assessment) . "', event_resources1='" . mysql_escape_string($file1) . "', event_resources2='" . mysql_escape_string($file2) . "', event_resources3='" . mysql_escape_string($file3) . "', event_begin='" . mysql_escape_string($begin) 
             . "', event_end='" . mysql_escape_string($end) . "', event_time='" . mysql_escape_string($time) . "', event_recur='" . mysql_escape_string($recur) . "', event_repeats='" . mysql_escape_string($repeats) . "', event_author=".$current_user->ID.", event_category=".mysql_escape_string($category).", event_link1='".mysql_escape_string($linky1)."',event_link2='".mysql_escape_string($linky2)."',event_link3='".mysql_escape_string($linky3)."'";
        ...
        // Add custom field start

        $sql = "SELECT event_id FROM " . WP_CALENDAR_TABLE . " ORDER BY event_id DESC LIMIT 1";
        $result = $wpdb->get_results($sql);
        $eventID = $result[0]->event_id;

        $noOfCustomRows = $_REQUEST['theValue'];

        if($noOfCustomRows == '' ) $noOfCustomRows = 10;

        for($custom=1;$custom<=$noOfCustomRows;$custom++){
             $custom_key = $_REQUEST["custom_key".$custom];
             $custom_val = $_REQUEST["custom_val".$custom];
             if($custom_key!='' && $custom_val!=''){
                 $sql = "INSERT INTO " . WP_CALENDAR_CUSTOM_TABLE . " SET event_id='" . $eventID . "', custom_key='" . mysql_escape_string($custom_key) . "', custom_value='" . mysql_escape_string($custom_val) . "'";
                 $wpdb->get_results($sql);
             }
        }

        // Add custom field end
	    ...
	    if ( empty($result) || empty($result[0]->event_id) )
	      {
                ?>
		<div class="error"><p><strong><?php _e('Error','calendar'); ?>:</strong> <?php _e('A lesson with the details you submitted could not be found in the database. This may indicate a problem with your database or the way in which it is configured.','calendar'); ?></p></div>
		<?php
	      }
	    else
	      {
		?>
		<div class="updated"><p><?php _e('Lesson added. It will now show in your calendar.','calendar'); ?></p></div>
		<?php
	      }
	  }
	else
	  {
		...
		$users_entries->event_objective = $objective;
		$users_entries->event_assessment = $assessment;
		$users_entries->event_resources1 = $file1;
		$users_entries->event_resources2 = $file2;
		$users_entries->event_resources3 = $file3;
	    ...
	    $users_entries->event_link1 = $linky1;
	    $users_entries->event_link2 = $linky2;
	    $users_entries->event_link3 = $linky3;
	  }
}
...
elseif ( $action == 'edit_save' )
{
	...
	$sql11 = "SELECT * FROM " . WP_CALENDAR_TABLE . " where event_id='".$event_id."'";
    ...
    if($_FILES["event_resources1"]["tmp_name"]!=''){
        ...
        move_uploaded_file($_FILES["event_resources1"]["tmp_name"],"../wp-content/" . $file1);
         ...
         $file1 = $result11[0]->event_resources1;
	}
    ...
    if($_FILES["event_resources2"]["tmp_name"]!=''){
        ...
        $file2 = $result11[0]->event_resources2;
	}
    ...
    if($_FILES["event_resources3"]["tmp_name"]!=''){
        ...
        $file3 = $result11[0]->event_resources3;
	}
	...
	{
		?>
		<div class="error"><p><strong><?php _e('Failure','calendar'); ?>:</strong> <?php _e("You can't update a lesson if you haven't submitted a lesson id",'calendar'); ?></p></div>
		<?php		
	}
	else
	{
	  ...
	  if ((preg_match($date_format_one,$begin) || preg_match($date_format_two,$begin)) && (preg_match($date_format_one,$end) || preg_match($date_format_two,$end)))
	      ...
	      if (checkdate($begin_m,$begin_d,$begin_y) && checkdate($end_m,$end_d,$end_y))
		{
		  // Ok, now we know we have valid dates, we want to make sure that they are either equal or that the end date is later than the start date
		    ...
		    {
                      ?>
                      <div class="error"><p><strong><?php _e('Error','calendar'); ?>:</strong> <?php _e('Your lesson end date must be either after or the same as your lesson begin date','calendar'); ?></p></div>
                      <?php
                    }
		}
	      else
	  ...
	  if (preg_match('/^[a-zA-Z0-9]{1}[a-zA-Z0-9[:space:]]{0,229}$/',$title))
	      ...
	      $title_ok =1;
            ...
            {
	      ?>
              <div class="error"><p><strong><?php _e('Error','calendar'); ?>:</strong> <?php _e('The lesson title must be between 1 and 230 characters in length and contain no punctuation. Spaces are allowed but the title must not start with one.','calendar'); ?></p></div>
              <?php
	    }
	  // We run some checks on recurrance              
          if (($repeats == 0 && $recur == 'S') || (($repeats >= 0) && ($recur == 'W' || $recur == 'M' || $recur == 'Y')))
	  ...
	  if ($start_date_ok == 1 && $end_date_ok == 1 && $time_ok == 1 && $url_ok == 1 && $title_ok && $recurring_ok == 1)
		...
		$sql = "UPDATE " . WP_CALENDAR_TABLE . " SET event_title='" . mysql_escape_string($title)
		     . "', event_desc='" . mysql_escape_string($desc) . "', event_objective='" . mysql_escape_string($objective) . "', event_assessment='" . mysql_escape_string($assessment) . "', event_resources1='" . mysql_escape_string($file1) . "', event_resources2='" . mysql_escape_string($file2) . "', event_resources3='" . mysql_escape_string($file3) . "', event_begin='" . mysql_escape_string($begin) 
                     . "', event_end='" . mysql_escape_string($end) . "', event_time='" . mysql_escape_string($time) . "', event_recur='" . mysql_escape_string($recur) . "', event_repeats='" . mysql_escape_string($repeats) . "', event_author=".$current_user->ID . ", event_category=".mysql_escape_string($category).", event_link1='".mysql_escape_string($linky1)."', event_link2='".mysql_escape_string($linky2)."', event_link3='".mysql_escape_string($linky3)."' WHERE event_id='" . mysql_escape_string($event_id) . "'";
		$wpdb->get_results($sql);
        ...
        $sql = "DELETE FROM " . WP_CALENDAR_CUSTOM_TABLE . " WHERE event_id='" . mysql_escape_string($event_id) . "'";
        $wpdb->get_results($sql);
        ...
        for($custom=1;$custom<=$noOfCustomRows;$custom++){
             ...
             $custom_val = $_REQUEST["custom_val".$custom];
             if($custom_key!='' && $custom_val!=''){
                 $sql = "INSERT INTO " . WP_CALENDAR_CUSTOM_TABLE . " SET event_id='" . $event_id . "', custom_key='" . mysql_escape_string($custom_key) . "', custom_value='" . mysql_escape_string($custom_val) . "'";
                 $wpdb->get_results($sql);
             }
        }
		...
		{
			?>
			<div class="updated"><p><?php _e('Lesson updated successfully','calendar'); ?></p></div>
			<?php
		}
	    }
          else
	    {
	      // The form is going to be rejected due to field validation issues, so we preserve the users entries here
              $users_entries->event_title = $title;
			  $users_entries->event_objective1 = $objective1;
		$users_entries->event_objective2 = $objective2;
		$users_entries->event_objective3 = $objective3;
		$users_entries->event_assessment = $assessment;
		$users_entries->event_resources = $file;
...
elseif ( $action == 'delete' )
{
	...
	{
		?>
		<div class="error"><p><strong><?php _e('Error','calendar'); ?>:</strong> <?php _e("You can't delete a lesson if you haven't submitted a lesson id",'calendar'); ?></p></div>
		<?php			
	}
	else
	{
		...
		{
			?>
			<div class="updated"><p><?php _e('Lesson deleted successfully','calendar'); ?></p></div>
			<?php
		}
		else
		{
...
<div class="wrap">
	...
	{
		?>
		<h2><?php _e('Edit Lesson','calendar'); ?></h2>
		<?php
		if ( empty($event_id) )
		{
			echo "<div class=\"error\"><p>".__("You must provide a lesson id in order to edit it",'calendar')."</p></div>";
		}
		else
		{
	...
	{
		?>
		<h2><?php _e('Add Lesson','calendar'); ?></h2>
		<?php wp_events_edit_form(); ?>
	
		<h2><?php _e('Manage Lessons','calendar'); ?></h2>
		<?php
...
function edit_calendar_config()
{
  ...
  <div class="wrap">
  <h2><?php _e('Lesson Options','calendar'); ?></h2>
  <form name="quoteform" id="quoteform" class="wrap" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=calendar-config">
                <div id="linkadvanceddiv" class="postbox">
                        <div style="float: left; width: 98%; clear: both;" class="inside">
                                <table cellpadding="5" cellspacing="5">
				<tr>
                                <td><legend><?php _e('Choose the lowest user group that may manage lessons','calendar'); ?></legend></td>
				<td>        <select name="permissions">
				            ...
				            <option value="admin" <?php echo $admin_selected ?>><?php _e('Administrator','calendar')?></option>
                                ...
                                <tr>
				<td><legend><?php _e('Do you want to display the author name on lessons?','calendar'); ?></legend></td>
...
<!--                                 <tr>
				<td><legend><?php _e('Display todays lessons?','calendar'); ?></legend></td>
						...
						<option value="off" <?php echo $no_disp_todays ?>><?php _e('No','calendar') ?></option>
                                ...
                                <tr>
				<td><legend><?php _e('Display upcoming lessons?','calendar'); ?></legend></td>
				    ...
				    <?php _e('for','calendar'); ?> <input type="text" name="display_upcoming_days" value="<?php echo $upcoming_days ?>" size="1" maxlength="2" /> <?php _e('days into the future','calendar'); ?>
                                ...
                                <tr>
				<td><legend><?php _e('Enable classes?','calendar'); ?></legend></td>
  ...
  if (isset($_POST['mode']) && $_POST['mode'] == 'add')
    {
      ...
      $wpdb->get_results($sql);
      echo "<div class=\"updated\"><p><strong>".__('Class added successfully','calendar')."</strong></p></div>";
    }
  else if (isset($_GET['mode']) && isset($_GET['category_id']) && $_GET['mode'] == 'delete')
    {
      ...
      $wpdb->get_results($sql);
      echo "<div class=\"updated\"><p><strong>".__('Class deleted successfully','calendar')."</strong></p></div>";
    }
  else if (isset($_GET['mode']) && isset($_GET['category_id']) && $_GET['mode'] == 'edit' && !isset($_POST['mode']))
    {
      ...
      ?>
<div class="wrap">
   <h2><?php _e('Edit Class','calendar'); ?></h2>
    <form name="catform" id="catform" class="wrap" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=calendar-categories">
				...
				<table cellpadding="5" cellspacing="5">
                                <tr>
				<td><legend><?php _e('Class Name','calendar'); ?>:</legend></td>
  ...
  else if (isset($_POST['mode']) && isset($_POST['category_id']) && isset($_POST['category_name']) && isset($_POST['category_colour']) && $_POST['mode'] == 'edit')
    {
      ...
      $wpdb->get_results($sql);
      echo "<div class=\"updated\"><p><strong>".__('Class edited successfully','calendar')."</strong></p></div>";
    }
...
?>
  ...
  <div class="wrap">
    <h2><?php _e('Add Class','calendar'); ?></h2>
    <form name="catform" id="catform" class="wrap" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=calendar-categories">
                ...
                <div id="linkadvanceddiv" class="postbox">
       				...
       				<table cellspacing="5" cellpadding="5">
                                ...
                                <tr>
                                <td><legend><?php _e('Class Color (Hex format)','calendar'); ?>:</legend></td>
                                <td><input type="text" name="category_colour" class="input" size="10" maxlength="7" value="" /><br />(ex. Blue - #ADD8E6, Green - #ADD8E6, Red - #CD5C5C, Orange - #FF8C00, Yellow - #F0E68C)</td>
                                </tr>
                                </table>
                        </div>
		        <div style="clear:both; height:1px;">&nbsp;</div>
    ...
    <h2><?php _e('Manage Classes','calendar'); ?></h2>
<?php
 ...
 if ( !empty($categories) )
   {
     ...
     <table class="widefat page fixed" width="50%" cellpadding="3" cellspacing="3">
       ...
       <tr>
         <th class="manage-column" scope="col"><?php _e('ID','calendar') ?></th>
	 <th class="manage-column" scope="col"><?php _e('Class Name','calendar') ?></th>
	 <th class="manage-column" scope="col"><?php _e('Class Color','calendar') ?></th>
	 <th class="manage-column" scope="col"><?php _e('Edit','calendar') ?></th>
	 <th class="manage-column" scope="col"><?php _e('Delete','calendar') ?></th>
       </tr>
       </thead>
   ...
   {
     echo '<p>'.__('There are no classes in the database - something has gone wrong!','calendar').'</p>';
   }
...
function draw_event($event)
{
  ...
  else { $linky = '?event_id='.$event->event_id; }

  $details = '<br />
* <span class="calnk" nowrap="nowrap"><a href="'.$linky.'" style="'.$style.'">' . $event->event_title . '</a></span>';
...
<table class="cat-key">
<tr><td colspan="2"><strong>'.__('Class Key','calendar').'</strong></td></tr>
';
...
function calendar_details(){
	...
	$sql = "SELECT * FROM " . WP_CALENDAR_TABLE . " WHERE event_id='" . $event_id . "' LIMIT 1";
	$result = $wpdb->get_results($sql);
...
if($result[0]->event_link1 !=''){
    $RelatedLinks.= '<a href="'.$result[0]->event_link1.'" target=_blank>'.$result[0]->event_link1.'</a><br>';
}
if($result[0]->event_link2 !=''){
    $RelatedLinks.= '<a href="'.$result[0]->event_link2.'" target=_blank>'.$result[0]->event_link2.'</a><br>';
}
if($result[0]->event_link3 !=''){
    $RelatedLinks.= '<a href="'.$result[0]->event_link3.'" target=_blank>'.$result[0]->event_link3.'</a><br>';
}

$ResourceFile = '';
if($result[0]->event_resources1 !=''){
    $ResourceFile.= '<a href="../wp-content/'.$result[0]->event_resources1.'" target=_blank>'.$result[0]->event_resources1.'</a><br>';
}

if($result[0]->event_resources2 !=''){
    $ResourceFile.= '<a href="../wp-content/'.$result[0]->event_resources2.'" target=_blank>'.$result[0]->event_resources2.'</a><br>';
}

if($result[0]->event_resources3 !=''){
    $ResourceFile.= '<a href="../wp-content/'.$result[0]->event_resources3.'" target=_blank>'.$result[0]->event_resources3.'</a><br>';
}
...
?>
  ...
  <TR>
	<TD width="30%" valign="top"><b>Lesson:</b></TD>
	<TD width="70%" valign="top"><?php echo $result[0]->event_title;?></TD>
  </TR>
  
  <?php
  $dt = explode("-",$result[0]->event_begin);
  $final_date = $dt[1].'-'.$dt[2].'-'.$dt[0];
  
  ?>
  
  <TR>
	<TD valign="top"><b>Date:</b></TD>
	<TD valign="top"><?php echo $final_date;?></TD>
  </TR>
  <TR>
	<TD valign="top"><b>Objective:</b></TD>
	<TD valign="top"><?php echo $result[0]->event_objective;?></TD>
  </TR>
  <TR>
	<TD valign="top"><b>Description:</b></TD>
	<TD valign="top"><?php echo $result[0]->event_desc;?></TD>
  </TR>
  <TR>
	<TD valign="top"><b>Assessment/Evaluation:</b></TD>
	<TD valign="top"><?php echo $result[0]->event_assessment;?></TD>
  </TR>

  <TR>
	<TD valign="top"><b>Related Links:</b></TD>
	<TD valign="top"><?php echo $RelatedLinks;?></TD>
  </TR>
  <TR>
	<TD valign="top"><b>Resources/Files:</b></TD>
	<TD valign="top"><?php echo $ResourceFile;?></TD>
  </TR>
  </TABLE>
